...
// order (affinity DESC) is predictable: fake-<type>-1 (strongest) → -3 (weakest).
// Fields cover the required trio (type/tmdb_id/rankScore) plus the optional pool
// columns upsertCandidates binds, so storage needs no backfill. tmdb_id is a
// STRING (contract); popularity is always present (upsert binds it directly).
function candidatesFor(type) {
  return [
    { type, tmdb_id: `fake-${type}-1`, rankScore: 3, imdb_id: `ttfake${type}1`, title: 'Alpha',   year: 2024, primary_genre: 'Drama',  genres: 'Drama',  vote_average: 8, vote_count: 5000, popularity: 30, reason: 'because Alpha', recCount: 2 },
    { type, tmdb_id: `fake-${type}-2`, rankScore: 2, imdb_id: `ttfake${type}2`, title: 'Bravo',   year: 2023, primary_genre: 'Comedy', genres: 'Comedy', vote_average: 7, vote_count: 4000, popularity: 20, reason: 'because Bravo', recCount: 1 },
    { type, tmdb_id: `fake-${type}-3`, rankScore: 1, imdb_id: `ttfake${type}3`, title: 'Charlie', year: 2022, primary_genre: 'Action', genres: 'Action', vote_average: 6, vote_count: 3000, popularity: 10, recCount: 3 },
  ];
}

// Build a conformant engine descriptor. `unrestricted` is the only axis that
// varies between the two fixtures; everything else is identical so a test can
// swap gated↔open and observe ONLY the I7 behaviour change.
function makeEngine({ id, name, unrestricted }) {
  return {
    id,
    name,
    ...
    async generate(profile, type, ctx) {
      ...
      const cands = candidatesFor(type);
...
// The canonical gated fixture (safe for any profile via the shared age gate).
